<html>
  <head>
    ...
    <g:javascript>
      ...
      function confirmDelivery(lId){
        if(confirm('Вы подтверждаете сдачу?'))
          <g:remoteFunction action='deliveryconfirm' onSuccess="\$('instruction_submit_button').click()" params="'id='+lId" />
      }
    </g:javascript>
  </head>
  <body onload="\$('instruction_submit_button').click()">
    <h1>${infotext?.header?:''}</h1>
    <g:rawHtml>${infotext?.itext?:''}</g:rawHtml>
    <g:formRemote class="contact-form" name="instructionForm" url="[action:'instructionlist']" update="[success:'resultlist']" onLoading="\$('loader').show()" onLoaded="\$('loader').hide()">
      <fieldset>
        ...
        <input type="submit" class="button" id="instruction_submit_button" value="Найти" style="margin-left:10px"/>
