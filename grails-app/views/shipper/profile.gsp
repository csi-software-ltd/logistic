    ...
    <g:javascript>
      function processResponse(e){
        if(e.responseJSON.error){
          var sErrorMsg = '';
          ['name','nickname','password1','password2','tel','tel1'].forEach(function(ids){
          ...
          e.responseJSON.errorcode.forEach(function(err){
            switch (err) {
              ...
              case 7: sErrorMsg+='<li>Некорректный формат телефона (+71111111111)</li>'; $("tel").addClassName('red'); break;
              case 8: sErrorMsg+='<li>Некорректный формат телефона2 (+71111111111)</li>'; $("tel1").addClassName('red'); break;
    ...
    <g:formRemote class="contact-form" name="userDetailForm" url="[action:'saveUserProfile']" method="post" onSuccess="processResponse(e)">
      <fieldset>
        ...
        <g:if test="${!user_internal && (user.is_emailcheck?:0)==0}">
          <a class="button" href="javascript:void(0)" onclick="sendConfirmationMail(this)" title="Подтвердить"><i class="icon-ok"></i></span></a>
        </g:if>          
          <label for="tel">Телефон:</label>
          <div class="input-append">            
            <input type="text" class="nopad normal" id="tel" name="tel" value="${user.tel}" autocomplete="off"/>
        ...
        <g:if test="${user.tel && (user.is_telcheck?:0)==0&&!isSMSsend}">
          ...
          <input type="text" id="tel1" name="tel1" value="${user.tel1}" autocomplete="off" />
        </div>
        <div class="grid_6 fleft">
